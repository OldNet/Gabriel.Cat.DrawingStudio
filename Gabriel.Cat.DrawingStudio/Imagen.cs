				                    	...
				                    	for(int i=0;i<fragments.Count;i++)
				                    	{
				                    		if(fragments[i].IsVisible){
				                    			fixed(byte* ptFragmento=fragments[i].RgbValues)
				                    				Gabriel.Cat.BitmapExtension.SetFragment(ptTotal,bmpTotal.Height,bmpTotal.Width,ISARGBBMPTOTAL,ptFragmento,fragments[i].Image.Height,fragments[i].Image.Width,ImageBase.ISARGB,rctImgResultado.GetRelativePoint(new Point(fragments[i].Location.X,fragments[i].Location.Y)));
				                    		}
				                    	}
